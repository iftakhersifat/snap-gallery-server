...
const tmpChunkDir = path.join(baseUploadDir, '_chunks'); // ✅ FIXED: persistent storage
...
// === Multer Setup for Direct Upload ===
...
const chunkUpload = multer({ storage: chunkStorage, limits: { fileSize: 100 * 1024 * 1024 } }); // 100MB max per chunk
const uploadNone = multer();

// === Routes ===
app.get('/', (req, res) => res.send('🚀 snap-gallery-server is running'));

// === Direct Upload (Single + Multi) ===
...
app.post('/upload/chunk', chunkUpload.single('chunk'), (req, res) => {
  if (!req.file) {
    console.error('❌ No chunk uploaded', req.body);
...
app.post('/upload/complete', uploadNone.none(), async (req, res) => {
  ...
  try {
    for (let i = 0; i < Number(totalChunks); i++) {
      ...
      console.log(`📁 Looking for chunk ${i}: ${chunkPath}`);
      ...
      fs.unlinkSync(chunkPath); // delete after merge
    }
    ...
    writeStream.end(async () => {
      ...
      const result = await mediaCollection.insertOne(mediaDoc);
      console.log('✅ File saved:', mediaDoc.url);
      res.json({ success: true, mediaId: result.insertedId, media: mediaDoc });
    });
...
app.delete('/media/:id', async (req, res) => {
  try {
    ...
    const filePath = path.join(baseUploadDir, media.url.replace(/^\/uploads\//, ''));
